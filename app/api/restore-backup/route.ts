  ...
  try {
    ...
    if (
      ...
      return Response.json(
        {
          error:
            "backup_path and restore_to are required",
        },
        { status: 400 }
      );
    }
    ...
    if (
      !isInsideData(body.backup_path) ||
      !isInsideData(body.restore_to)
    ) {
      return Response.json(
        {
    ...
    if (
      !fs.existsSync(body.backup_path)
    ) {
      return Response.json(
        {
          error:
